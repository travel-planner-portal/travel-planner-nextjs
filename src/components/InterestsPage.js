...
const InterestsPage = ({ inputData }) => {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { setPlanData } = useNavigationContext();
  const [tripType, setTripType] = useState(inputData?.tripType || "");
  const [interests, setInterests] = useState(inputData?.interest || []);
  ...
  const onBack = () => {
    ...
    if (inputData.path === "Destination") {
      console.log("inputData.destination", inputData.destination);
      router.push("/discover/dates");
    } else {
      router.push("/");
    }
  };
  ...
  const handleProceed = () => {
    if (!tripType) {
      ...
      // Preserve existing userData while adding new data
      const currentUserData = useSelector((state) => state.userData);
      ...
      if (inputData.path === "Destination") {
        router.push(
          `/discover/trips/${inputData.destination}/${inputData.duration}`
        );
      } else {
  ...
  const handleSkip = () => {
    if (inputData.path === "Discover") {
      ...
      navigate(`/discover/trips/${inputData.destination}`);
    }
  };
  ...
  return (
    <div className="container background-image-homepage flex flex-col items-center max-w-[88rem] md:w-[80%] mx-auto px-4 md:px-6 pb-6 pt-[9px] md:pt-[28px]">
      ...
      <div className="flex flex-row items-center gap-6 md:gap-14 bottom-8 md:bottom-[15vh] mt-16">
        {/* <button
          onClick={handleSkip}
          className="hover:text-gray-900 text-[16px] md:text-[20px] font-rubikregular_400 text-darkBlack"
        >
          Skip
        </button> */}
        <button
